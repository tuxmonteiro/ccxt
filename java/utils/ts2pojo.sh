...
ls -1 ${JSON_DIR} | sed 's/.json//' | while read class; do

  # https://github.com/joelittlejohn/jsonschema2pojo/releases
  echo -e 'package dev.tuxmonteiro.jccxt.base.types;\n' > ${JAVA_DIR}/$class.java
  ./jsonschema2pojo-1.2.2/bin/jsonschema2pojo -o -sl -s ${JSON_DIR}/$class.json -t ${JSON_DIR} -log trace
  [ -f ${JSON_DIR}/$class.java ] && cat ${JSON_DIR}/$class.java >> ${JAVA_DIR}/$class.java || rm -vf ${JSON_DIR}/$class.java ${JAVA_DIR}/$class.java
  echo
done
