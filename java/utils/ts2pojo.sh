...
ls -a ${JSON_DIR} | grep '.java' | sed 's/.java//' | while read class; do
  echo -e 'package dev.tuxmonteiro.jccxt.base.types;\n' | tee ${JAVA_DIR}/$class.java
  cat ${JSON_DIR}/$class.java | tee -a ${JAVA_DIR}/$class.java
